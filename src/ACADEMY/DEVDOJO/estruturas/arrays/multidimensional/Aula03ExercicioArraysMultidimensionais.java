package ACADEMY.DEVDOJO.estruturas.arrays.multidimensional;
...
public class Aula03ExercicioArraysMultidimensionais {
    public static void main(String[] args) {
        ...
        arrayMulti1[2][1] = 5;
        arrayMulti1[2][2] = 21;
        int resultMultipDiagonalPrincipal = 1;
        for (int i = 0; i < arrayMulti1.length; i++) {
            for (int j = 0; j < arrayMulti1[i].length; j++) {
                if (i == j) {
                    resultMultipDiagonalPrincipal = resultMultipDiagonalPrincipal * arrayMulti1[i][j];
                }
            }
        }
        System.out.println(resultMultipDiagonalPrincipal);
    }
}
